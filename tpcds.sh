...
check_variables()
{
	...
	if [ "$count" -eq "0" ]; then
		echo "REPO_URL=\"https://github.com/southpolekat/TPC-DS\"" >> $MYVAR
		new_variable=$(($new_variable + 1))
	fi
...
echo su --session-command="cd \"$INSTALL_DIR/$REPO\"; ./rollout.sh $GEN_DATA_SCALE $EXPLAIN_ANALYZE $RANDOM_DISTRIBUTION $MULTI_USER_COUNT $RUN_COMPILE_TPCDS $RUN_GEN_DATA $RUN_INIT $RUN_DDL $RUN_LOAD $RUN_SQL $RUN_SINGLE_USER_REPORT $RUN_MULTI_USER $RUN_MULTI_USER_REPORT $RUN_SCORE $SINGLE_USER_ITERATIONS $COMPRESSTYPE" $ADMIN_USER 
